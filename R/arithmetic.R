...
expected_colnames <- c("date", "suburb", "postcode", "offence_level_1", "offence_level_2",
 ...
 # datacolumns <- colnames(crime_data)
  ...
  postcodes_new <- postcodes[2]

  ggplot(plot_data, aes(x=month) , fill=c(x,y)) +
    geom_line(aes(y = plot_data$x, colour = postcodes))+
    geom_line(aes(y = plot_data$y, colour = postcodes_new))+
   #
    labs(x = "Timeline(Month of a Date)" scale_x_continuous(breaks= c(1:12)))
